...
The options endpoint accepts `search`, `page`, `per_page`, and `selected` query parameters and returns:
...
`selected` is used to hydrate the visible label when a table opens with an active URL filter. Backend option requests use normal Laravel logging and error handling; avoid logging raw search terms or selected values in custom resolvers.
